...
const findBones = (pwd) => {
  ...
  if (isDirectory(bones)) {
    ...
    console.log(
      "👉 If this is your first time using bones, run `bones init` in your project to get started."
    );
    process.exit(1);
  } else {
...
const copyRecursive = (sourcePath, destPath, log = false) => {
  if (isDirectory(sourcePath)) {
    ...
    fs.readdirSync(sourcePath).forEach((subpath) =>
      copyRecursive(
...
const processTemplateString = (path, templateArgs) =>
  Object.keys(templateArgs).reduce((s, key) => {
    ...
    return s.replace(search, (match, fn) =>
      fn ? templateHelpers[fn](templateArg) : templateArg
    );
  }, path);
